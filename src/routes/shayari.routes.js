...
// Engagement (any logged-in user)
router.post("/:id/like", authenticate, shayariController.like);
router.post("/:id/bookmark", authenticate, shayariController.bookmark);
router.post("/:id/share", shayariController.share);

// Public detail (last, so it doesn't shadow the routes above)
router.get("/:slug", optionalAuth, shayariController.getBySlug);
